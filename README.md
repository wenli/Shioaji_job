...
* **資料庫自動初始化與路徑統一**：系統啟動時會自動檢查並在您於 `.env` 中設定的 `DB_NAME` 資料庫路徑上完成 K 線表格初始化（支援讀取全域環境變數）。
* **強健的行情模擬與時區安全**：後端模擬看盤已改為與真實時鐘分鐘跨越對齊，解決了每秒產生新 K 棒的問題；前端交易終端亦全面採用安全台北時區（Asia/Taipei）以防 Session 錯亂。
