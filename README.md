...
1.  **🗂️ 多週期支援 (Multi-Timeframes)**
    ...
    *   其中 **60K (台指期 60 分K)** 採用客製化**日盤與夜盤獨立 Resample 聚合邏輯**：
        *   **日盤 (08:45 ~ 14:00)**：使用 `closed="right", label="right", offset="45min"` 對齊，開盤 `08:45` 單分 K 獨立對齊標記為 `08:45:00`，首根完整 60 分 K 區間標記為 `09:45:00`。
        *   **夜盤 (15:00 ~ 隔日 08:44)**：使用 `closed="right", label="right"` 對齊，開盤 `15:00` 單分 K 獨立對齊標記為 `15:00:00`，首根完整 60 分 K 區間標記為 `16:00:00`。
        *   **無時間戳平移**：最終輸出時間戳完全對齊原始區間，確保不發生額外平移偏移。
