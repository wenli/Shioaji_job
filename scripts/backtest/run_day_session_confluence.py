# -*- coding: utf-8 -*-
"""
台指期【純日盤專屬】銀色子彈 Killzone + SMC 訂單塊 (Order Block) 策略回測報告
- 口數限制: 嚴格上限 2 口 (Max Lots = 2)
- 階梯停利: TP1 2.0R (平1口移保本) / TP2 3.0R (剩餘1口全平)
- 停損保底: 25.0 點
- 評估範圍: 2024-01-01 至 2026-09-16 全歷史 (MTX 小台指) - 僅限日盤 (Day Session)
...
class DaySessionOrderBlockEngine(OrderBlockEngine):
    """
    純日盤專屬 OrderBlockEngine，支援日盤各細分時段精確過濾
    """
    def __init__(self, df_1k: pd.DataFrame, df_5k: pd.DataFrame, config: OrderBlockConfig, day_mode: str = "day_ob_golden"):
        super().__init__(df_1k, df_5k, config)
        self.day_mode = day_mode

    def is_in_day_window(self, dt: pd.Timestamp) -> Tuple[bool, str]:
        t = dt.time()

        if self.day_mode == "day_sb_kz":
            # 1. 日盤銀色子彈 Killzone (09:00 - 10:00)
            if time(9, 0) <= t < time(10, 0):
                return True, "DAY_SB_0900_1000"
            return False, ""

        elif self.day_mode == "day_ob_golden":
            # 2. 日盤訂單塊黃金窗口 (10:00 - 11:59)
            if time(10, 0) <= t < time(12, 0):
                return True, "DAY_OB_1000_1159"
            return False, ""

        elif self.day_mode == "day_combined_0911":
            # 3. 日盤銀色子彈+訂單塊整合 (09:00 - 11:59)
            if time(9, 0) <= t < time(12, 0):
                return True, "DAY_COMBINED_0900_1159"
            return False, ""

        elif self.day_mode == "day_trend_1013":
            # 4. 日盤盤中順勢波段 (10:00 - 13:30)
            if time(10, 0) <= t < time(13, 30):
                return True, "DAY_TREND_1000_1330"
            return False, ""

        elif self.day_mode == "day_full":
            # 5. 日盤全時段 (08:45 - 13:45)
            if time(8, 45) <= t <= time(13, 45):
                return True, "DAY_FULL_0845_1345"
            return False, ""
    ...
    def run_backtest(self) -> Tuple[Dict[str, Any], List[Dict[str, Any]], List[Dict[str, Any]]]:
        ...
        for i in range(1, n):
            ...
            # A. 持倉處理 (TP1 平1口移保本 / TP2 剩餘1口全平 / 日盤收盤強制離場)
            if position != 0:
            ...
            # B. 尋找純日盤指定時段內的 Order Block 回踩訊號
            if position == 0:
                if curr_sess != 'day':
                    continue

                in_window, win_tag = self.is_in_day_window(t_cur)
                if not in_window:
                    continue
...
def generate_day_session_html_report(df_1k, results: List[Dict[str, Any]], output_file: str):
    """生成精美純日盤回測對比報告"""
    ...
    # 找出表現最佳的策略交易流水
    best_exp = max(results, key=lambda x: x['summary']['net_profit'])
    best_trades = best_exp['trades'][:100]
...
<head>
    ...
    <title>SMC 台指【純日盤專屬】銀色子彈 vs 訂單塊 回測評比報告</title>
    ...
    <style>
        ...
        * {{ margin: 0; padding: 0; box-sizing: border-box; font-family: 'Outfit', sans-serif; }}
        ...
        th {{ color: var(--text-muted); padding: 12px 14px; border-bottom: 1px solid var(--border); font-weight: 600; }}
        ...
        .highlight-row {{ background: rgba(16, 185, 129, 0.06); }}
    </style>
</head>
<body>
    <div class="container">
        <header>
            <h1>☀️ SMC 台指期 <span>【純日盤專屬】量化回測報告</span></h1>
            <p class="sub">合約標的: 小台指 (MTX) | 歷史區間: 2024-01-01 ~ 2026-09-16 | 嚴格 2 口限制、2.0R/3.0R 階梯停利 (TP1平1口移保本/TP2全平)</p>
        </header>

        <div class="card">
            <h2 style="font-size: 18px; margin-bottom: 16px;">📊 日盤各細分時段績效矩陣</h2>
            <table>
                <thead>
                    <tr>
                        <th>策略時段方案</th>
...
"""
    for exp in results:
        s = exp['summary']
        is_best = (exp['name'] == best_exp['name'])
        row_cls = 'class="highlight-row"' if is_best else ''
        pnl_class = "val-profit" if s['net_profit'] > 0 else "val-loss"
        html += f"""
                    <tr {row_cls}>
                        <td style="font-weight: 700; color: #fff;">{'🏆 ' if is_best else ''}{exp['name']}</td>
                        <td style="color: var(--text-muted);">{exp['window_desc']}</td>
                        <td>{s['total_trades']} 筆</td>
                        <td>{s['win_rate']:.1f}%</td>
                        <td style="color: #60a5fa; font-weight: bold;">{s['profit_factor']:.2f}</td>
                        <td class="{'val-profit' if s['max_drawdown_pct']<8 else 'val-loss'}">{s['max_drawdown_pct']:.1f}%</td>
    ...
    html += f"""
        ...
        <div class="card">
            <h2 style="font-size: 18px; margin-bottom: 16px;">📈 日盤各時段資金走勢曲線 (Equity Curves)</h2>
        ...
        <div class="card">
            <h2 style="font-size: 18px; margin-bottom: 16px;">📋 最佳日盤策略【{best_exp['name']}】交易明細流水 (Top 100 Exits)</h2>
            <table>
                <thead>
                    <tr>
                        <th>#</th>
...
"""
    for t in best_trades:
        ...
        html += f"""
                    <tr>
                        <td>{t['trade_no']}</td>
                        <td><span class="badge" style="background: rgba(59,130,246,0.2); color:#93c5fd;">{t['stage']}</span></td>
...
"""
    with open(output_file, "w", encoding="utf-8") as f:
        f.write(html)
    print(f"\n[純日盤 HTML 報告已生成]: {os.path.abspath(output_file)}")


def main():
    db_path = get_db_path()
    print("=" * 85)
    print(f"啟動台指期【純日盤 (Day Session)】銀色子彈 vs 訂單塊 回測評估 (2口限制、2.0R/3.0R 階梯停利)")
    print("=" * 85)
    ...
    # 5 大純日盤細分時段配置
    experiments = [
        ("【日盤黃金窗口】訂單塊純順勢 (首選)", "day_ob_golden", "10:00 - 11:59 (避開開盤噪聲，現貨主力趨勢明朗)"),
        ("【日盤銀色子彈】開盤爆發 Killzone", "day_sb_kz", "09:00 - 10:00 (台股現貨剛開盤時段)"),
        ("【日盤早盤整合】銀彈開盤 + 訂單塊", "day_combined_0911", "09:00 - 11:59 (現貨開盤 + 盤中前半場)"),
        ("【日盤盤中波段】10點後持續至尾盤", "day_trend_1013", "10:00 - 13:30 (趨勢確立後一路做到尾盤)"),
        ("【日盤全時段開】日盤開盤至收盤", "day_full", "08:45 - 13:45 (整個日盤無時段過濾)"),
    ]

    results = []

    print("\n" + "=" * 85)
    print(f"{'日盤策略配置':<32} | {'總淨利 (NTD)':<14} | {'PF':<6} | {'MDD':<6} | {'勝率':<6} | {'交易數':<6}")
    print("-" * 85)

    for name, day_mode, window_desc in experiments:
        engine = DaySessionOrderBlockEngine(df_1k, df_5k, base_cfg, day_mode=day_mode)
        ...
        print(f"{name:<30} | NT$ {summary['net_profit']:>+9,.0f} | {summary['profit_factor']:>4.2f} | {summary['max_drawdown_pct']:>4.1f}% | {summary['win_rate']:>4.1f}% | {summary['total_trades']:>4} 筆")
    ...
    output_html = os.path.join(os.path.dirname(__file__), "../../reports/sb_order_block_day_session_2lots.html")
    generate_day_session_html_report(df_1k, results, output_html)
